...
class ControllerSaleCustomer extends Controller {
	...
	protected function getList() {
		...
		$filter_data = array(
			...
			'filter_user_group'		   => $filter_user_group,// Kit Add 23/3/2016
			'filter_telephone'		   => $filter_telephone,// Kit Add 23/3/2016
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'name'           => $result['name'],
				'email'          => $result['email'],
				'informtype'     => $result['informtype'], //Kit Add
				'telephone'      => $result['telephone'],//Kit Add
				'mobile'         => $result['mobile'],//Kit Add
				'fax'			 => $result['fax'],//Kit Add
				'custom_field'	 => substr($result['custom_field'], strpos($result['custom_field'], '"')+ 1,(strrpos($result['custom_field'], '"')-stripos($result['custom_field'], '"'))-1),//Kit Add
		...
		$data['column_telephone'] = $this->language->get('column_telephone');//Kit Add
		$data['column_informtype'] = $this->language->get('column_informtype');//Kit Add

		$data['entry_name'] = $this->language->get('entry_name');
		$data['entry_email'] = $this->language->get('entry_email');
		$data['entry_telephone'] = $this->language->get('entry_telephone');//Kit Add
		...
		$data['sort_telephone'] = $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&sort=c.telephone' . $url, 'SSL'); //Kit Add
	...
	protected function getForm() {
		...
		$data['entry_informtype'] = $this->language->get('entry_informtype');		
		...
		$data['button_history_add'] = $this->language->get('button_Cust_history_add'); // Kit add 9/4/2016
		...
		$data['tab_history'] = $this->language->get('tab_cust_history');
		...
		if (isset($this->request->get['filter_user_group'])) {
			$data['filter_user_group'] = $this->request->get['filter_user_group'];
		} else {
			$data['filter_user_group'] = 0;
		}
		
		if (isset($this->session->data['user_id'])) {
			$data['filter_user_id'] = $this->session->data['user_id'];
		} else {
			$data['filter_user_id'] = null;
		}
		// End Add
		...
		if (isset($this->request->post['informtype'])) {
			$data['informtype'] = $this->request->post['informtype'];
		} elseif (!empty($customer_info)) {
			$data['informtype'] = $customer_info['informtype'];
		} else {
			$data['informtype'] = '';
		}
	...
	public function history() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateHistory()) {
			//$this->model_sale_customer->addHistory($this->request->get['customer_id'], $this->request->post['comment'], $this->request->post['user_id']); // one remark
			$this->model_sale_customer->addHistory($this->request->get['customer_id'], $this->request->post['comment'], $this->request->post['user_id'], $this->request->post['order_id']); // one add
		...
		foreach ($results as $result) {
			$data['histories'][] = array(
				...
				'user_id'     => $result['user_id'], //Kit Add 25/3/2016
				'user_info'     => $result['user_info'], //Kit Add 25/3/2016
				'order_id'     => $result['order_id'], //one add 
				//'date_added'  => date($this->language->get('date_format_short'), strtotime($result['date_added'])) // one remark
				'date_added' => date($this->language->get('datetime_format'), strtotime($result['date_added']))  // one add
			);
		}
