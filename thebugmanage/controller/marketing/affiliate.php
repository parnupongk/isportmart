...
class ControllerMarketingAffiliate extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['affiliates'][] = array(
				...
				'name'         => $result['name'],
				'email'        => $result['email'],
				'affiliatetype'        => $result['affiliatetype'], // one add @ 2016-05-18
				'dnis'        => $result['dnis'], // one add @ 2016-05-18
				'commission'        => $result['commission'], // one add @ 2016-05-18
	...
	protected function getForm() {
		...
		if (isset($this->request->post['dnis'])) {  // one add @ 2016-05-18
			$data['dnis'] = $this->request->post['dnis'];
		} elseif (!empty($affiliate_info)) {
			$data['dnis'] = $affiliate_info['dnis'];
		} else {
			$data['dnis'] = '';
		} 
		
		if (isset($this->request->post['affiliatetype'])) {  // one add @ 2016-05-18
			$data['affiliatetype'] = $this->request->post['affiliatetype'];
		} elseif (!empty($affiliate_info)) {
			$data['affiliatetype'] = $affiliate_info['affiliatetype'];
		} else {
			$data['affiliatetype'] = 'VENDOR';
		} 
