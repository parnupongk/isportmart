				...
				if (file_exists($file)) {
					...
					if ($zip) {
						...
						while ($entry = zip_read($zip)) {
							...
							// Compare thebugmanage files
							$file = DIR_APPLICATION . substr($zip_name, 13);

							if (is_file($file) && substr($zip_name, 0, 13) == 'upload/thebugmanage/') {
				...
				if ($login) {
					...
					if ($root) {
						foreach ($files as $file) {
							...
							// Many people rename their thebugmanage folder for security purposes which I believe should be an option during installation just like setting the db prefix.
							...
							if (substr($destination, 0, 5) == 'thebugmanage') {
