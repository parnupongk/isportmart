...
class ModelReportCustomer extends Model {
	...
	public function getCustomerHistory($data = array()) {
		...
		//$sql = "SELECT CONCAT(c.firstname, ' ', c.lastname,' : ', c.telephone) customer_name, ca.customer_history_id, ca.customer_id, ca.comment, ca.date_added, ca.user_id, ca.order_id FROM ";
		//$sql .=  DB_PREFIX . "customer_history ca LEFT JOIN " . DB_PREFIX . "customer c ON (ca.customer_id = c.customer_id)";
		
		$sql = "SELECT CONCAT(c.firstname, ' ', c.lastname,' : ', c.telephone) customer_name, ca.customer_history_id, ca.customer_id, ca.comment, ca.date_added, ca.user_id, ca.order_id, u.firstname user_info FROM ";
		$sql .=  DB_PREFIX . "customer_history ca LEFT JOIN " . DB_PREFIX . "customer c ON (ca.customer_id = c.customer_id) LEFT JOIN oc_user u ON (ca.user_id = u.user_id)";		
		...
		if (!empty($data['filter_comment'])) {
			$implode[] = "ca.comment LIKE '%" . $this->db->escape($data['filter_comment']) . "%'";
		}
	...
	public function getTotalCustomerHistory($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "customer_history` ca LEFT JOIN " . DB_PREFIX . "customer c ON (ca.customer_id = c.customer_id)";
