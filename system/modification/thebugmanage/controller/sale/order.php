...
//				curl_setopt($curl, CURLOPT_CAINFO,  '/var/www/ssl-cert/ca.crt'); 
//				curl_setopt($curl, CURLOPT_SSLCERT, '/var/www/ssl-cert/server.pem'); 

//				curl_setopt($curl, CURLOPT_CAINFO,  '/var/www/ssl-cert/DigiCertCA.crt');
//				curl_setopt($curl, CURLOPT_SSLCERT, '/var/www/ssl-cert/outdigitca.pem'); 
...
//				curl_setopt($curl, CURLOPT_SSLCERT, '/var/www/ssl-cert/outdigitca.pem'); 
		...
		if (isset($response['success'])) {
			...
			$url = '';
			//nid add 18/3/2016 11:15
			if (isset($this->session->data['user_group'])) {
				$url .= '&filter_user_group=' . $this->session->data['user_group'];
			}
    ...
    if (isset($this->session->data['user_group'])) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('common/dashboard', 'token=' . $this->session->data['token'].'&filter_user_group=' . $this->session->data['user_group'], 'SSL')
		);
		...
		$filter_data = array(
			...
			'filter_telephone'         => $filter_telephone,  // one add
			'filter_email'         => $filter_email, // one add
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'order_status_id'        => $result['order_status_id'],  // one add 
				'total'         => $this->currency->format($result['total'], $result['currency_code'], $result['currency_value']),
				'date_added'    => date($this->language->get('datetime_format'), strtotime($result['date_added'])),
				'date_modified' => date($this->language->get('datetime_format'), strtotime($result['date_modified'])),
				
				'dnis' => $result['dnis'],   // koy add
				'phone' => $result['phone'],   // one add
				'email' => $result['email'],   // one add
				'invoice_no' => $result['inv_no'],   // one add 				
				'affiliate_name' => $result['affiliate_name'],   // one add 
				...
				'complaint' => $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&order_id=' . $result['order_id'], 'SSL'),  //one add
		...
		$data['entry_email'] = $this->language->get('entry_email');  // one add
		$data['entry_telephone'] = $this->language->get('entry_telephone');  // one add
		...
		$data['button_complaint'] = $this->language->get('button_complaint');  //one add
    ...
    if (isset($this->session->data['user_group'])) {
		...
		$data['sort_cust_taxid'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=cust_taxid' . $url, 'SSL');
		$data['sort_affiliate_name'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=affiliate_name' . $url, 'SSL');
		$data['sort_dnis'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=dnis' . $url, 'SSL');
		$data['sort_inv_no'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=inv_no' . $url, 'SSL');
    ...
    if (isset($this->session->data['user_group'])) {
		...
		//nid add 18/03/2016 11:43
		$data['filter_user_group'] = $filter_user_group;
		$data['filter_order_id'] = $filter_order_id;
		$data['filter_customer'] = $filter_customer;
		$data['filter_order_status'] = $filter_order_status;
		//$data['filter_total'] = $filter_total;
		$data['filter_email'] = $filter_email;		// one add
		$data['filter_telephone'] = $filter_telephone;		// one add
...
//echo "<pre>";print_r($data);
	...
	public function getForm() {
		...
		// Kit Add 19/4/2016
		if (isset($this->session->data['user_group'])) {
			$data['filter_user_group'] = $this->session->data['user_group'];
		} else {
			$data['filter_user_group'] = 0;
		}
    ...
    if($seller_details AND isset($seller_details['name']) AND $seller_details['name'])
      $product['name'] = $product['name'].' by Seller <b>'.$seller_details['name'].'</b>';                     
			...
			//nid add 21/03/2016 16:04     
$data['confirm_agent_id']	= '';	 		
$data['confirm_date_confirm']	= '';	 
$data['confirm_total']	= '';				
$data['confirm_type']	= '';					
$data['confirm_remark']		= '';			
$data['confirm_picname'] = '';				
$data['confirm_pic_date_added']= '';
//nid add 21/03/2016 16:04     
		}
		...
		$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();
		$data['order_statuses_seq'] = $this->model_localisation_order_status->getOrderStatuses_seq( $data['order_status_id'] ); //one add
    ...
    //  print_r($order_info);	
		if ($order_info) {
			...
			$data['text_loading'] = $this->language->get('text_loading');
			//nid add 22/03/2016 10:20
			$data['text_agent_id'] 			=$this->language->get('text_agent_id'); 
			...
			$data['text_payslip']				=$this->language->get('text_payslip');
			$data['text_notfound']			=$this->language->get('text_notfound');
			...
			//nid add 21/03/2016 15:50
			$data['tab_verifier'] = $this->language->get('tab_verifier');
			//nid add 21/03/2016 15:50
			$data['token'] = $this->session->data['token'];
			...
			$data['address'] = $this->url->link('sale/order/address', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			...
			$data['complaint'] = $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');  //one add
			$data['cancel'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . $url, 'SSL');

			$data['order_id'] = $this->request->get['order_id'];
			//nid add 18/3/2016 15:34
			if (isset($this->session->data['user_group'])) {
			$data['filter_user_group'] = $this->session->data['user_group'];
		} else {
			$data['filter_user_group'] = '';
		}
		if (isset($this->request->get['filter_order_status'])) {
			...
			$data['affiliate_telephone'] = $order_info['affiliate_telephone'];
      ...
      $data['confirm_agent_id']				= $order_info['confirm_agent_id'];				
			$data['confirm_date_confirm']		= $order_info['confirm_date_confirm'];		
			$data['confirm_total']					= $order_info['confirm_total'];					
			$data['confirm_type']						= $order_info['confirm_type'];						
			$data['confirm_remark']					= $order_info['confirm_remark'];					
			$data['confirm_picname'] 				= $order_info['confirm_picname']; 				
			$data['confirm_pic_date_added'] = $order_info['confirm_pic_date_added']; 
      //nid add 21/03/2016 16:04
			// Custom fields
			...
			$data['order_comment'] = $order_info['comment'];
    ...
    $filter_data = array(
		...
		$data['order_statuses_seq'] = $this->model_localisation_order_status->getOrderStatuses_seq( $order_info['order_status_id'] ); //one add
	...
	public function history() {
		...
		foreach ($results as $result) {
			$data['histories'][] = array(
				...
				'status'     => $result['status'],
				'user_info'     => $result['user_info'], // one add
				'comment'    => nl2br($result['comment']),
				//nid edit 21/3/2016 11:27
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$data['orders'][] = array(
					...
					'store_owner'        => $store_info['config_owner']				 // one add
				);
			}
		}

		$this->response->setOutput($this->load->view('sale/order_address.tpl', $data));
	}
	...
	public function api() {
		$this->load->language('sale/order');
		//$log = new Log('apidebug201608.log');
		...
		if ($this->validate()) {
			...
			if (isset($this->session->data['cookie']) && isset($this->request->get['api'])) {
				...
				//$log->write( "api url = " . $url . 'index.php?route=' . $this->request->get['api'] . ($url_data ? '&' . http_build_query($url_data) : '') );
			...
			//$log->write( "api err = " . $response['error'] );
