...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$data['entry_affiliate'] = $this->language->get('entry_affiliate');  // one add
		...
		$data['filter_affiliate_id'] = $filter_affiliate_id;   // one add
		$data['filter_affiliate'] = $filter_affiliate;   // one add
	...
	protected function getForm() {
		...
		$data['entry_store'] = $this->language->get('entry_store');  // name 
		...
		$data['entry_recurring'] = $this->language->get('entry_recurring');
			...
			$data['entry_warrantyday'] = $this->language->get('entry_warrantyday');
		...
		// -- one add 1/2 -begin
			...
			//print_r( $this->request->post['affiliate_id'] );
			
			if (isset($this->request->post['affiliate_id'])) {
				$data['affiliate_id'] = $this->request->post['affiliate_id'];
			} elseif (!empty($product_info)) {
				$data['affiliate_id'] = $product_info['affiliate_id'];
			//} else {
			//	$data['affiliate_id'] = 0;
			}
			...
			if (isset($this->request->post['affiliate'])) {
				...
				$affiliate_info = $this->model_marketing_affiliate->getAffiliate($product_info['affiliate_id']);
			...
			// -- one add 2/2 -begin
			if (isset($this->request->post['warrantyday'])) {
				$data['warrantyday'] = $this->request->post['warrantyday'];
			} elseif (!empty($product_info)) {
		...
		// one add - begin
		if (isset($this->request->post['marginvalue'])) {
			$data['marginvalue'] = $this->request->post['marginvalue'];
		} elseif (!empty($product_info)) {
		...
		if (isset($this->request->post['margintype'])) {
			$data['margintype'] = $this->request->post['margintype'];
		} elseif (!empty($product_info)) {
			$data['margintype'] = $product_info['margintype'];
		} else {
			$data['margintype'] = '';
		}				
		// one add - end
		...
		foreach ($product_options as $product_option) {
          ...
          if($data['wkget_timezone']){
              ...
              $data['entry_sdate'] = $this->language->get('entry_sdate');
              $data['entry_auction'] = $this->language->get('entry_auction');
              $data['entry_acution_button'] = $this->language->get('entry_acution_button');
              $data['entry_isacution'] = $this->language->get('entry_isacution');
              $data['entry_quantitylimit'] = $this->language->get('entry_quantitylimit');
              $data['entry_voucherlimit'] = $this->language->get('entry_voucherlimit');
              $data['help_quantitylimit'] = $this->language->get('help_quantitylimit');
              $data['help_voucherlimit'] = $this->language->get('help_voucherlimit');            
             ...
             if (isset($this->request->get['product_id'])) {
                ...
                foreach ($product_auctions as $product_auction) {
                           $data['isauction']=$product_auction['isauction'];
                           $data['auction_min']=$product_auction['min'];
                           $data['auction_max']= $product_auction['max'];
                           $data['auction_end']=$product_auction['end_date'];
                           $data['auction_name']=$product_auction['name'];
                           $data['auction_start']=$product_auction['start_date'];
                           $data['product_quantity_limit']=$product_auction['quantity_limit'];
                           $data['voucher_expiry']=$product_auction['voucher_time_limit'];
                      }
                }

                if (isset($this->request->post['isauction'])) {
                  $data['isauction'] = $this->request->post['isauction'];
                }

                if (isset($this->request->post['auction_name'])) {
                  $data['auction_name'] = $this->request->post['auction_name'];
                }

                if (isset($this->request->post['auction_min'])) {
                    $data['auction_min'] = $this->request->post['auction_min'];
                }

                if (isset($this->request->post['auction_max'])) {
                    $data['auction_max'] = $this->request->post['auction_max'];
                }

                if (isset($this->request->post['auction_end'])) {
                    $data['auction_end'] = $this->request->post['auction_end'];
                }

                if (isset($this->request->post['auction_start'])) {
                    $data['auction_start'] = $this->request->post['auction_start'];
                }
                ...
                if (isset($this->request->post['voucher_expiry'])) {
                    $data['voucher_expiry'] = $this->request->post['voucher_expiry'];
                }

                if(isset($this->error['error_auction_min'])) {
                  $data['error_auction_min'] = $this->error['error_auction_min'];
                } else {
                  $data['error_auction_min'] = '';
                }

                if(isset($this->error['error_auction_max'])) {
                  $data['error_auction_max'] = $this->error['error_auction_max'];
                } else {
                  $data['error_auction_max'] = '';
                }

                 if(isset($this->error['error_auction_start'])) {
                  $data['error_auction_start'] = $this->error['error_auction_start'];
                } else {
                  $data['error_auction_start'] = '';
                }

                if(isset($this->error['error_auction_end'])) {
                  $data['error_auction_end'] = $this->error['error_auction_end'];
                } else {
                  $data['error_auction_end'] = '';
                }

                if(isset($this->error['error_voucher_expiry'])) {
                  $data['error_voucher_expiry'] = $this->error['error_voucher_expiry'];
                } else {
                  $data['error_voucher_expiry'] = '';
                }
	...
	protected function validateForm() {
		...
		if ($this->error && !isset($this->error['warning'])) {
			$this->error['warning'] = $this->language->get('error_warning');
                   ...
                   if ($this->request->post['isauction']) {
                        
                          if(isset($this->request->post['auction_min']) && !$this->request->post['auction_min']) {
                            $this->error['warning'] = $this->language->get('error_warning');
                            $this->error['error_auction_min'] = $this->language->get('error_auction_min');
                          }

                          if(isset($this->request->post['auction_max']) && !$this->request->post['auction_max']) {
                            $this->error['warning'] = $this->language->get('error_warning');
                            $this->error['error_auction_max'] = $this->language->get('error_auction_max');
                          }

                          if(!isset($this->error['error_auction_min']) && !isset($this->error['error_auction_max']
                            ) && $this->request->post['auction_max'] <= $this->request->post['auction_min']) {
                            $this->error['warning'] = $this->language->get('error_warning');
                            $this->error['error_auction_max'] = $this->language->get('error_auction_minmax'); 
                          }

                          if(isset($this->request->post['auction_start']) && !$this->request->post['auction_start']) {
                            $this->error['warning'] = $this->language->get('error_warning');
                            $this->error['error_auction_start'] = $this->language->get('error_auction_start');
                          }

                          if(isset($this->request->post['auction_end']) && !$this->request->post['auction_end']) {
                            $this->error['warning'] = $this->language->get('error_warning');
                            $this->error['error_auction_end'] = $this->language->get('error_auction_end');
                          }

                          if(!isset($this->error['error_auction_start']) && !isset($this->error['error_auction_end']
                            ) && $this->request->post['auction_start'] >= $this->request->post['auction_end']) {
                            $this->error['warning'] = $this->language->get('error_warning');
                            $this->error['error_auction_end'] = $this->language->get('error_auction_startend'); 
                          }

                          if(!isset($this->error['error_voucher_expiry']) && !isset($this->error['error_voucher_expiry']
                            ) && $this->request->post['auction_end'] >= $this->request->post['voucher_expiry']) {
                            $this->error['warning'] = $this->language->get('error_warning');
                            $this->error['error_voucher_expiry'] = $this->language->get('error_voucher_expiry'); 
                          }
                      }
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_affiliate_id'])) { // one add
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');
			...
			$filter_data = array(
				'filter_name'  => $filter_name,
				'filter_model' => $filter_model,
				'filter_affiliate_id' => $filter_affiliate_id,  // one add // not specific, will not where cause
				'start'        => 0,
				'limit'        => $limit
			);
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					...
					'model'      => $result['model'],
					'option'     => $option_data,
					'affiliate_id' => $result['affiliate_id'], // one add @ 12/07/2016
					'price'      => $result['price']
				);
			}
		}
