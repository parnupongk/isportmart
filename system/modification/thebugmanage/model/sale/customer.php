...
class ModelSaleCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', mobile = '" . $this->db->escape($data['mobile']) . "', fax = '" . $this->db->escape($data['fax']) . "', informtype = '" . $this->db->escape($data['informtype']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function deleteCustomer($customer_id) {
                         ...
                         if($this->config->get('wkproduct_auction_status')){
                           $this->db->query("DELETE FROM " . DB_PREFIX . "wkauctionbids WHERE user_id = '" . (int)$customer_id . "'");
                           $this->db->query("DELETE FROM " . DB_PREFIX . "wk_automatic_auctionbids WHERE user_id = '" . (int)$customer_id . "'");
                         }
	...
	public function getCustomers($data = array()) {
		...
		if (!empty($data['filter_telephone'])) {
			$implode[] = "CONCAT(c.telephone, ' ', c.mobile,' ', c.fax,' ', c.custom_field) LIKE '%" . $this->db->escape($data['filter_telephone']) . "%'";
		}
		// Kit End 
	...
	public function getTotalCustomers($data = array()) {
		...
		// Kit Add 25/3/2016
		if (!empty($data['filter_telephone'])) {
			$implode[] = "CONCAT(telephone, ' ', mobile,' ', fax,' ', custom_field) LIKE '%" . $this->db->escape($data['filter_telephone']) . "%'";
		}
		// Kit End 
		if (!empty($data['filter_name'])) {
	...
	//public function addHistory($customer_id, $comment, $user_id) {
	public function addHistory($customer_id, $comment, $user_id, $order_id) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer_history SET order_id=" .(int)$order_id. ", customer_id = '" . (int)$customer_id . "', comment = '" . $this->db->escape(strip_tags($comment)) . "', user_id = '". $this->db->escape($user_id)."', date_added = NOW()"); // Kit Add 25/3/2016
	}
	...
	public function getHistories($customer_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT ca.order_id, ca.comment, ca.date_added, ca.user_id, u.firstname as user_info FROM " . DB_PREFIX . "customer_history ca LEFT JOIN " . DB_PREFIX . "customer c ON (ca.customer_id = c.customer_id) LEFT JOIN ". DB_PREFIX ."user u ON (ca.user_id = u.user_id) WHERE c.customer_id = '" . (int)$customer_id . "' ORDER BY c.date_added DESC LIMIT " . (int)$start . "," . (int)$limit); // Kit Add 25/3/2016
