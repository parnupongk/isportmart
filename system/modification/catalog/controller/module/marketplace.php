...
class ControllerModuleMarketplace extends Controller { 
	...
	public function index() {
		...
		if(isset($this->request->get['route']) AND (substr($this->request->get['route'],0,8)=='account/')) {
	    	...
	    	$data['moreProductUrl'] = $this->url->link('account/customerpartner/productlist', '', '');
            ...
            if($this->config->get('wkproduct_auction_status')) {
                ...
                $data['account_menu_href']['auction'] = $this->url->link('account/customerpartner/wk_mpaddauction','','SSL');
            }else{
                $data['wkproduct_auction_status'] = false;
                if(isset($data['account_menu_href']['auction'])) {
                    unset($data['account_menu_href']['auction']);
                }
