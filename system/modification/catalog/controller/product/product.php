...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
            ...
            $chk_auction = $this->model_module_wkproduct_auction->getAuction($this->request->get['product_id']);
            foreach ($chk_auction as $key => $value) {
              if(isset($value['product_id'])){
                          if($this->config->get('wkproduct_auction_cartstatus') && $this->config->get('wkproduct_auction_status')){
                          $data['wk_cart_option'] = true;
                          }
            }
          }
		...
		if ($product_info) {
			...
			$data['affiliate'] = $this->model_catalog_product->getAffName($product_info['affiliate_id']);
			...
			if (!empty($this->session->data['cart'])) {
				$firstkey = unserialize(base64_decode(key($this->session->data['cart'])));
				//print_r($firstkey);
				$data['fst_affiliate'] = $this->model_catalog_product->getAffProduct(reset($firstkey));
			}else {
				$data['fst_affiliate'] = '0';
			}			
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'affiliate'  => $this->model_catalog_product->getAffName($result['affiliate_id']),
					'thumb'       => $image,
					'name'        => $result['name'],
                                    ...
                                    if (!$this->config->get('marketplace_seller_info_by_module') && !$this->config->get('marketplace_seller_info_hide')) {
                                        ...
                                        if ($check_seller) {
                                            ...
                                            switch ($this->config->get('marketplace_product_name_display')) {
                                                ...
                                                case 'cn':
                                                    $data['info_name'] = $partner['company'];
                                                    break;

                                                case 'sncn':
                                                    $data['info_name'] = $partner['firstname']. ' ' .$partner['lastname'].' '.'And'.' '.$partner['company'];
                                                    break;  
                                            }
                                            ...
                                            $getAllAverageFeedback = $this->model_customerpartner_master->getAllAverageFeedback($check_seller['customer_id']);

                                            $data['info_avgprice']   = $getAllAverageFeedback['avgprice'];
                                            $data['info_avgvalue']   = $getAllAverageFeedback['avgvalue'];
                                            $data['info_avgquality'] = $getAllAverageFeedback['avgquality'];
		...
		/*if ( $this->config->get('config_template') == 'default' ) {
			if (empty($this->session->data['agent_id'])) { // one add				
	...
	public function review() {
		...
		} elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/review.tpl')) {
