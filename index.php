...
if (isset($request->get['route'])) {
	// one add @ 18/8/59 -begin
	if (strpos($_SERVER['HTTP_HOST'], 'callcenter.isportmart') !== false ) {
		if (  $request->get['route'] == 'account/account/signbyagent'){
			$action = new Action($request->get['route']);
		} elseif ( isset( $session->data['agent_id'] ) )  {
			$action = new Action($request->get['route']);
		} else {
			$action = new Action('account/error');  // one add @ 18/8/59
		}		
	} else {  
		$action = new Action($request->get['route']);   // org.
	}
	// one add @ 18/8/59 - end
} else {
//	if (strpos($_SERVER['HTTP_HOST'], 'callcenter.') !== false ) { // one add @ 18/8/59
//		$action = new Action('account/error');  // one add @ 18/8/59
//	} else {
//		$action = new Action('common/home');  // one add @ 16/12/58	
//	}	
//	$action = new Action('common/home');  // one add @ 16/12/58	
	header('location:https://'.$_SERVER['HTTP_HOST'] .'/?route=common/home');	
