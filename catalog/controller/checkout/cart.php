...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			foreach ($products as $product) {
				if ($aff_id == '0') {
					$aff_id = $product['affiliate_id'];
				}elseif ($aff_id != $product['affiliate_id']) {
					$chk_aff = '1';
				}
			...
			if ($chk_aff == '1') {
				$data['error_warning'] = $this->language->get('error_affiliate');
			}
			...
			if (isset($this->session->data['agent_id'])) {
				...
				$this->model_account_activity->logActivity('checkout/cart','cart', $log_data);

			} elseif (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/cart.tpl')) {
	...
	public function add() {
		...
		if (!empty($this->session->data['cart'])) {
			...
			//print_r($firstkey);
			$fst_affiliate = $this->model_catalog_product->getAffProduct(reset($firstkey));
		}else {
			$fst_affiliate = '0';
		}	
		...
		if ($product_info) {
		if (($fst_affiliate == '0')||($product_info['affiliate_id'] == $fst_affiliate)) {
			...
			if (!$json) {
				...
				$this->model_account_activity->logActivity('checkout/cart/add',' product add', $log_data);
	...
	public function edit() {
		...
		if (!empty($this->request->post['quantity'])) {
			...
			$this->model_account_activity->logActivity('checkout/cart/edit','product edit', $log_data);
	...
	public function remove() {
		...
		if (isset($this->request->post['key'])) {
			...
			$this->model_account_activity->logActivity('checkout/cart/remove','product remove', $log_data);
