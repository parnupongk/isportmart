...
class ModelAccountCustomer extends Model {
	...
	public function editCustomer($data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', mobile = '" . $this->db->escape($data['mobile']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "'");

		$this->event->trigger('post.customer.edit', $customer_id);
	}

	// koy - 30/10/2015 Firstname, Lastname, Email, Telephone, Fax, Mobile
	public function editCustomerFLETF($data) {
		$this->event->trigger('pre.customer.edit', $data);

		$customer_id = $this->customer->getId();

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', mobile = '" . $this->db->escape($data['mobile']) . "', informtype = '" . $this->db->escape($data['inform']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		
		$this->event->trigger('post.customer.edit', $customer_id);
	}
	
	// koy - 30/10/2015 Email, Telephone, Fax, Mobile
	public function editCustomerETF($data) {
		$this->event->trigger('pre.customer.edit', $data);

		$customer_id = $this->customer->getId();

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET email = '" . $this->db->escape(utf8_strtolower($data['email'])) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', mobile = '" . $this->db->escape($data['mobile']) . "', informtype = '" . $this->db->escape($data['inform']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	// one add - begin 
	public function getCustomerByANI($ani) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer where custom_field like '%". $ani . "%' ");
		return $query->row;
	}
	
	public function getCustomerByOrder($order) {
		$query = $this->db->query("SELECT cust.* FROM " . DB_PREFIX . "customer as cust," . DB_PREFIX . "order as ord where cust.customer_id = ord.customer_id and ord.order_id = '". $order . "' ");
		if ($query->num_rows) {
			return $query->row;
		} else {
			return 0;
	...
	public function addCustomerByANI($ani, $agent_id) {
		//$this->event->trigger('pre.customer.add', $data);

		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = 2, store_id =0, " . 
			" firstname = '" . $ani . "', lastname = '" . $ani . "', email = '" . $ani . "@ishop.com', telephone = '" . $ani . 
			"', custom_field = 'a:1:{i:1;s:". strlen($ani) .":\"" . $ani . "\";}" .
			"', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . 
			"', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($ani)))) . 
			"', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . 
			"', status = '1', approved = '1', date_added = NOW(), agent_id='" . $agent_id ."' " );
			
		$customer_id = $this->db->getLastId();

		$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . 
			"', firstname = '" . $ani . "', lastname = '" . $ani . "', address_1 = '" . $ani . 
			"', city = '" . $ani . 
			"', country_id = 209,  zone_id = 3192 ");
		$address_id = $this->db->getLastId();
...
//		$query = $this->db->query(" select c.category_id, count(*)  from oc_order a, oc_order_product b, oc_product_to_category c " .
// 					" where a.order_id=b.order_id and b.product_id=c.product_id and a.customer_id ='". $customer_id . "' group by c.category_id order by count(*) desc limit 1 ");
		$query = $this->db->query(" select count(*) as total from oc_order   where customer_id ='". $customer_id . "'   ");
		return $query->row['total'];
	}	
	// one add - end 
	//nid add
	...
	//nid add 24/03/2016 14:42
	public function getCustomerHistory($customer_id) {
		if(!empty($customer_id)){
			$condition = " where customer_id ='".(int)$customer_id . "'";		
			}
		//$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_history ".$condition);
		//where customer_id ="'".(int)$customer_id . "'";			
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_history where customer_id ='".(int)$customer_id . "'");			
		return $query->row;
	}
	
	//koy add 11/04/2016
	public function getUserid($agent_id) {			
...
//			(strpos( $ipAddress, '192.168.64.') !== false ) ) { 
		...
		$query = $this->db->query("SELECT user_id FROM " . DB_PREFIX . "user where agent_id ='" . $agent_id . "'");			
		if ($query->num_rows) {
