...
class ModelAccountAddress extends Model {
	...
	public function getAddresses() {
		...
		$query = $this->db->query("SELECT ad.*, cs.informtype  FROM " . DB_PREFIX . "address as ad," . DB_PREFIX . "customer cs WHERE ad.customer_id = cs.customer_id and ad.customer_id = '" . (int)$this->customer->getId() . "'");
		...
		foreach ($query->rows as $result) {
			...
			if ($result['informtype'] == 'S'){
				$informtype = 'SMS';
			} elseif ($result['informtype'] == 'E'){
				$informtype = 'Email';
			} else {
				$informtype = '';
			}
	...
	//add for query address_id edit address - koy - 30/10/2015
	public function getAddressbyTel($telephone) {
		$query = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "address WHERE address_1 = '" . $telephone . "'");
		
		if (isset($query->row['address_id'])) {
			$value = $query->row['address_id'];			
		} else {
			$value = '';
		}
