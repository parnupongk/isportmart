<?php 
class ModelCustomerpartnerHtmlfilter extends Model {


    private function tln_tagprint($tagname, $attary, $tagtype){
        if ($tagtype == 2) {
            $fulltag = '</' . $tagname . '>';
        } else {
            $fulltag = '<' . $tagname;
     ...
     */
    private function tln_skipspace($body, $offset){
     ...
     */
    private function tln_findnxstr($body, $offset, $needle){
     ...
     */
     private function tln_findnxreg($body, $offset, $reg){
        $matches = array();
        $retarr = array();
        $preg_rule = '%^(.*?)(' . $reg . ')%s';
     ...
     */
    private function tln_getnxtag($body, $offset){
     ...
     */
    private function tln_deent(&$attvalue, $regex, $hex = false){
     ...
     */
    private function tln_defang(&$attvalue){
        /**
     ...
     * @return     Void, modifies a reference value.
     */
    private function tln_unspace(&$attvalue){
        if (strcspn($attvalue, "\t\r\n\0 ") != strlen($attvalue)) {
    ...
    private function tln_fixurl($attname, &$attvalue, $trans_image_path, $block_external_images){
        $sQuote = '"';
    ...
    private function tln_fixstyle($body, $pos, $trans_image_path, $block_external_images){
        $me = 'tln_fixstyle';
    ...
    private function tln_body2div($attary, $trans_image_path){
        $me = 'tln_body2div';
