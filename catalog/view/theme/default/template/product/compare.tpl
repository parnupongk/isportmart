        ...
        <tr>
          ...
          <?php foreach ($products as $product) { ?>
			...
			<?php if (!empty($agent_id)) { ?>			  
				<input type="button" value="<?php echo $button_cart; ?>" class="btn btn-primary btn-block" onclick="addCart();" />
			<?php } else { ?>
				<a href="Tel:025026505">
              <button type="button" id="button-call" data-loading-text="Call 025026505" class="btn btn-primary btn-block">โทร 025026505</button>
            	</a>
			<?php } ?><br>
...
function addCart() {
	alert('<?php echo $fst_affiliate; ?>');
	alert('<?php echo $products[$product['product_id']]['affiliate_id']; ?>');
	<?php if ($fst_affiliate == 0) { ?>
		cart.add('<?php echo $product['product_id']; ?>', '<?php echo $product['minimum']; ?>');
	<?php } else { ?>
		<?php if ($fst_affiliate == $products[$product['product_id']]['affiliate_id']) { ?>
			cart.add('<?php echo $product['product_id']; ?>', '<?php echo $product['minimum']; ?>');	
		<?php } else { ?>
