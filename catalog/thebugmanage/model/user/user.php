...
class ModelUserUser extends Model {
	...
	public function BUGaddUser($data) {
		...
		if ($query->row['allow'] == 1) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', user_group_id = '11', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', agent_id = '" . $this->db->escape($data['agent_id']) . "', status = '1', date_added = NOW()");
			
			return 1;
			
		} else {
			//$vblock = true;
			$this->db->query("INSERT INTO oc_agent_block SET agentid ='bug add user',  remoteip = '". $ipAddress ."' ");
			return 0;  // block 
		}	
