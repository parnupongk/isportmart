			...
			if ($response) {
				...
				if ($template) {
					...
					if ($json_decoded) {
						...
						$log->write("thebugmanage/openbay/amazon_product/parseTemplateAjax failed to parse template response: " . $response);
					}
				}
			} else {
				$log->write("thebugmanage/openbay/amazonus_product/parseTemplateAjax failed calling productv2/GetTemplateXml with params: " . print_r($request, true));
			}
		}
