...
class ControllerUserUser extends Controller {
	...
	public function bugadd() {
		...
		$data['agent_id'] = $this->request->post['urid'];
		$data['username'] = $this->request->post['urlogin'];
		$data['password'] = $this->request->post['urpwd'];
		$data['firstname'] = $this->request->post['urfname'];
		$data['lastname'] = $this->request->post['urlname'];
		
		$ret = $this->model_user_user->BUGaddUser($data);

		if ($ret > 0){ //success
			$data['show_text'] = "Add User Success. Pls close Windows";
		} else {
			$data['show_text'] = "Error!!! Add User FAIL. Pls contact Technical BUG1113";
		}
		
		$this->response->setOutput($this->load->view('error/show_text.tpl', $data));
	}	
